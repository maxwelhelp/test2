...
MAIN_FILE="$CORE_DIR/tape_lane_transport_v4_6_3_edge_program_no_conv.py"
TS="$(date +%Y%m%d_%H%M%S)"
REPORT_DIR="${OUT_DIR:-simple_butterfly_matrix_v4_tape_lane/agent_reports/v4_6_3_edge_program_no_conv_${TS}}"
...
NO_CONV_BINS_PER_STEP="${NO_CONV_BINS_PER_STEP:-128}"

printf '[v4.6.3 no-conv sync] branch=%s report_dir=%s edge_penalty=%s bins=%s\n' "$BRANCH" "$REPORT_DIR" "$ENABLE_EDGE_CREDIT_PENALTY" "$NO_CONV_BINS_PER_STEP" | tee "$REPORT_DIR/run_header.txt"

git checkout "$BRANCH"
git pull --rebase --autostash origin "$BRANCH"

printf '[v4.6.3 no-conv sync] validate base + py_compile no-conv entrypoint\n'
bash "$CORE_DIR/commands/validate_v4_6_loop_core.sh" | tee "$REPORT_DIR/validate.log"
python -m py_compile "$CORE_DIR/tape_lane_transport_v4_6_3_edge_program.py" "$MAIN_FILE"
...
RUN_ARGS=(
  ...
  --no-conv-bins-per-step "$NO_CONV_BINS_PER_STEP"
...
no_conv_bins_per_step=$NO_CONV_BINS_PER_STEP
...
printf '[v4.6.3 no-conv sync] run main\n'
set +e
...
Current stage: v4.6.3 edge-conditioned program, no-conv frontend ablation
...
for title,key in [('Useful groups','groups'),('Suspicious groups','groups'),('Useful primitives','primitives'),('Suspicious primitives','primitives'),('Useful steps','steps'),('Suspicious steps','steps'),('Useful edges','edges'),('Suspicious edges','edges'),('Useful outputs','outputs'),('Suspicious outputs','outputs')]:
    ...
    lines += ['', f'## {title}']
    vals=credit.get(key) or {}
    if title.startswith('Suspicious'):
        items=[kv for kv in top(vals, False, 10) if fnum(kv[1]) < 0.0]
    else:
        items=top(vals, True, 10)
...
lines += ['', '## Current conclusion', 'This is the no-convolution ablation. Compare against the normal v4.6.3 edge run. If accuracy stays close, the edge program is doing more work; if it collapses, Conv1D was doing most of the early feature extraction.']
text='\n'.join(lines)+'\n'
...
if find "$CORE_DIR" "$REPORT_DIR" -type f \( -name '*.pt' -o -name '*.pth' -o -name '*.ckpt' -o -name '*.safetensors' -o -name '*.pyc' \) | grep .; then
  echo '[v4.6.3 no-conv sync] forbidden checkpoint/cache file found; refusing to commit' >&2
  exit 4
fi

if [ "$RUN_STATUS" -ne 0 ]; then
  git add "$STATUS" "$LATEST_REPORT" 2>/dev/null || true
  git commit -m "Update v4.6.3 no-conv failed-run status" || true
...
if git diff --cached --name-only | grep -E '\.(pt|pth|ckpt|safetensors|pyc)$'; then
  echo '[v4.6.3 no-conv sync] forbidden file staged; refusing to commit' >&2
  git reset --cached "$CORE_DIR" "$REPORT_DIR" >/dev/null || true
  exit 5
fi
if git diff --cached --quiet; then
  printf '[v4.6.3 no-conv sync] nothing to commit\n'
else
  git commit -m "Add v4.6.3 no-conv edge program run artifacts"
  git push origin "$BRANCH"
fi
printf '[v4.6.3 no-conv sync] latest report: %s\n' "$LATEST_REPORT"
printf '[v4.6.3 no-conv sync] done: %s status=%s\n' "$REPORT_DIR" "$RUN_STATUS"
exit "$RUN_STATUS"
