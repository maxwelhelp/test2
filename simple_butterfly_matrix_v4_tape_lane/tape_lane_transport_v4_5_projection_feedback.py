...
Two scout layers influence the v4.4 context-controller bridge:

1. Offline Projection Council feedback from scout_runtime_feedback_epoch_XXX.json.
   This is a frozen, tiny prior plus optional auxiliary target. It is not a
   permanent edit of the learned weights.

2. Online Scout: a tiny trainable MLP over current gctx inside forward. It runs
   right before boundary/transition/fanout/route choices and receives gradient
   through the real task loss.

Safety fixes in this version:
- no L2 gate penalty that collapses the scout gate to zero by default;
- consistent dtype casts at logit application sites;
- boundary hard-normalization stays removed, but boundary budget/aux losses stay
  active as prices;
- skip cost uses a safe local proxy;
- patch anchors are checked so silent string-patch failures become errors.
"""
...
def _translate_args_v45(argv: list[str]):
    ...
    env["SCOUT_FEEDBACK_SCALE"] = env.get("SCOUT_FEEDBACK_SCALE", "0.010")
    env["SCOUT_FEEDBACK_MODE"] = env.get("SCOUT_FEEDBACK_MODE", "detached_bias_plus_aux")
    env["SCOUT_AUX_LAMBDA"] = env.get("SCOUT_AUX_LAMBDA", "0.001")
    env["ONLINE_SCOUT_SCALE"] = env.get("ONLINE_SCOUT_SCALE", "0.012")
    env["ONLINE_SCOUT_GATE_INIT"] = env.get("ONLINE_SCOUT_GATE_INIT", "-2.2")
    # Default 0.0: do not crush the gate before it proves useful. Set manually if needed.
    env["ONLINE_SCOUT_USE_COST"] = env.get("ONLINE_SCOUT_USE_COST", "0.0")
    env["ONLINE_SCOUT_GATE_TARGET"] = env.get("ONLINE_SCOUT_GATE_TARGET", "0.10")
...
def _must_replace(text: str, old: str, new: str, name: str, required: bool = True) -> str:
    if old in text:
        return text.replace(old, new, 1)
    if required and os.environ.get("V45_ASSERT_PATCHES", "1") != "0":
        raise RuntimeError(f"[v4.5 patch] required anchor not found: {name}")
    return text
...
def _patch_wrapper_text_v45(text: str) -> str:
    ...
    repl = needle + (
        ...
        '        self.online_scout_gate_target = float(os.environ.get("ONLINE_SCOUT_GATE_TARGET", "0.10"))\n'
    ...
    if "self.online_scout_scale" not in text:
        text = _must_replace(text, needle, repl, "scout controls after alive_controller_scale")

    # 2) Online scout network. Use self.lanes instead of fragile local l*l.
    ...
    basis_repl = basis_anchor + (
        ...
        '        self.online_scout_net = nn.Sequential(nn.LayerNorm(4 * d), nn.Linear(4 * d, d), nn.SiLU(), nn.Linear(d, 1 + 8 + 4 + self.lanes * self.lanes))\n'
        '        self.online_scout_gate_logit = nn.Parameter(torch.tensor(float(os.environ.get("ONLINE_SCOUT_GATE_INIT", "-2.2"))))\n'
        '        _online_last = self.online_scout_net[-1]\n'
        '        nn.init.normal_(_online_last.weight, std=1e-4)\n'
        '        nn.init.zeros_(_online_last.bias)'
    )
    if "self.online_scout_net" not in text:
        text = _must_replace(text, basis_anchor, basis_repl, "online scout net after transition_route_basis")

    # 3) Primitive controller receives offline scout primitive bias.
    ...
    # 4) Boundary: remove hard per-step mean normalization, add offline+online scout bias.
    old_boundary_block = (
    ...
    new_boundary_block = (
        ...
        '            online_scout_gate_f = torch.sigmoid(self.online_scout_gate_logit.float()) if online_scout_logits is not None else torch.zeros((), device=x.device, dtype=torch.float32)\n'
        ...
        '                except Exception:\n'
        '                    scout_boundary_bias = 0.0\n'
        '            boundary_raw = torch.sigmoid(self.boundary_logit[t].to(device=x.device, dtype=x.dtype) + self.boundary_controller_scale * boundary_bias + self.scout_feedback_scale * scout_boundary_bias + self.online_scout_scale * online_scout_gate_f.to(dtype=x.dtype) * online_boundary_bias)  # [B]\n'
        '            boundary = boundary_raw.clamp(0.0, 1.0)  # [B], budget is enforced by losses/aux targets, not hard flattening'
    )
    if old_boundary_block in text:
        text = text.replace(old_boundary_block, new_boundary_block, 1)
    else:
        # If another patch already replaced boundary_raw, do not silently continue without online_scout_logits.
        if "online_scout_logits = self.online_scout_net" not in text and os.environ.get("V45_ASSERT_PATCHES", "1") != "0":
            raise RuntimeError("[v4.5 patch] boundary block anchor not found")

    hard_budget = (
        '            boundary_budget = torch.as_tensor(self.boundary_mean_target, device=x.device, dtype=boundary_raw.dtype)\n'
        '            boundary = (boundary_raw * boundary_budget / boundary_raw.detach().mean().clamp_min(1e-4)).clamp(0.0, 1.0)  # [B]'
    )
    if hard_budget in text:
        text = text.replace(hard_budget, '            boundary = boundary_raw.clamp(0.0, 1.0)  # [B], budget is enforced by losses/aux targets')
    ...
    trans_inject = trans_anchor + (
        ...
        '                transition_type_logits = transition_type_logits + self.online_scout_scale * online_scout_gate_f * online_scout_logits[:, 1:9].float()\n'
        '            if getattr(self, "scout_feedback_active", False):\n'
        '                try:\n'
    ...
    route_inject = route_anchor + (
        '            if online_scout_logits is not None:\n'
        '                _or = online_scout_logits[:, 13:].view(x.shape[0], self.lanes, self.lanes).to(dtype=route_logits.dtype)\n'
        '                route_logits = route_logits + self.online_scout_scale * online_scout_gate_f.to(dtype=route_logits.dtype) * _or\n'
        '            if getattr(self, "scout_feedback_active", False):\n'
        '                try:\n'
    ...
    fan_inject = fan_anchor + (
        '            if online_scout_logits is not None:\n'
        '                fanout_logits = fanout_logits + self.online_scout_scale * online_scout_gate_f * online_scout_logits[:, 9:13].float()\n'
        '            if getattr(self, "scout_feedback_active", False):\n'
        '                try:\n'
    ...
    # 9) Paid usage and skip cost. Default scout cost is 0; if enabled, target gate instead of crushing it to zero.
    skip_zero = 'out["skip_cost"] = torch.zeros((), device=logits.device)'
    skip_real = '_res_proxy = residual_proxy if "residual_proxy" in locals() else torch.zeros((), device=logits.device)\n    out["skip_cost"] = float(os.environ.get("SCOUT_SKIP_COST_SCALE", "0.001")) * torch.clamp(_res_proxy - float(os.environ.get("SKIP_COLLAPSE_TARGET", "4.0")), min=0.0).pow(2)'
    ...
    train_inject = train_anchor + (
        ...
        '                _gtarget = float(os.environ.get("ONLINE_SCOUT_GATE_TARGET", "0.10"))\n'
        '                loss = loss + float(os.environ.get("ONLINE_SCOUT_USE_COST", "0.0")) * (_gate - _gtarget).pow(2).to(device=logits.device)\n'
        '                _sl = float(os.environ.get("SCOUT_AUX_LAMBDA", "0.0"))\n'
        '                if _sl > 0.0 and hasattr(model.backbone, "scout_feedback") and model.backbone.scout_feedback:\n'
        '                    _bt = model.backbone.scout_feedback.get("boundary_target", [])\n'
        '                    if _bt and hasattr(baux, "boundaries") and baux.boundaries.numel():\n'
        '                        _target = torch.tensor([float(x) for x in _bt], device=baux.boundaries.device, dtype=baux.boundaries.float().dtype)\n'
        '                        _n = min(int(_target.numel()), int(baux.boundaries.numel()))\n'
        '                        if _n > 0:\n'
        '                            loss = loss + _sl * F.mse_loss(baux.boundaries[:_n].float(), _target[:_n].float())\n'
        '            except Exception:\n'
        '                pass\n'
    )
    if train_anchor in text and "SCOUT_AUX_LAMBDA" not in text:
    ...
    new_ctx = '"boundary_route_gate_scale": float(os.environ.get("BOUNDARY_ROUTE_GATE_SCALE", "0.25")), "alive_scale": float(os.environ.get("ALIVE_CONTROLLER_SCALE", "0.0")), "scout_feedback_path": os.environ.get("SCOUT_FEEDBACK_PATH", ""), "scout_feedback_scale": float(os.environ.get("SCOUT_FEEDBACK_SCALE", "0.0")), "scout_aux_lambda": float(os.environ.get("SCOUT_AUX_LAMBDA", "0.0")), "online_scout_scale": float(os.environ.get("ONLINE_SCOUT_SCALE", "0.0")), "online_scout_use_cost": float(os.environ.get("ONLINE_SCOUT_USE_COST", "0.0"))}'
    if old_ctx in text:
