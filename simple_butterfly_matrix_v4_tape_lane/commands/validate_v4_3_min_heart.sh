...
grep -q "def _candidate_key" "$FILE"
...
if grep -q "canonicalize_v4_3_main.py" "$SYNC"; then
  echo "[validate v4.3] ERROR: standard sync must not run canonicalizer"
  exit 1
fi
...
if grep -q "run_v4_3_min_heart_audit_fixed.sh" "$SYNC"; then
  echo "[validate v4.3] ERROR: standard sync must not run audit wrapper"
  exit 1
fi
...
echo "[validate v4.3] grad sanity must not run canonicalizer"
if grep -q "canonicalize_v4_3_main.py" "$GRAD"; then
  echo "[validate v4.3] ERROR: grad sanity must not run canonicalizer"
  exit 1
fi
