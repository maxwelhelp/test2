#!/usr/bin/env python3
"""v4.2: tape-lane router matrix program with weak priors and ablation controls.

Changes vs v4.1:
  - keeps the v4 tape/lane idea, no hard extract/compare/suppress/aggregate phases;
  - makes structured input/head/read/route priors weak and configurable;
  - adds neutral/structured ablation modes;
  - reduces default head lane prior strength;
  - fixes memory overwrite loss to penalize excessive memory writing, not all memory writing;
  - adds context-aware primitive logits inside transform.
"""
...
def resize_cells(x: torch.Tensor, cells: int) -> torch.Tensor:
    """Resize [B,E,D] evidence/state cells to [B,cells,D]."""
...
def class_lane_prior(classes: int, lanes: int, mode: str, init_strength: float) -> torch.Tensor:
    """Small initial head/lane prior. This is not a fixed assignment."""

    p = torch.zeros(int(classes), int(lanes))
    if mode == "neutral" or float(init_strength) <= 0.0:
        return p
    if mode not in {"round_robin", "soft_cover"}:
        raise ValueError(f"unknown class_lane_prior_mode={mode}")
    for c in range(int(classes)):
        if mode == "round_robin":
            ...
            # Softer coverage: every class sees every lane, with a tiny class-specific tilt.
            p[c, :] = 0.10
            p[c, c % int(lanes)] += 0.20
            p[c, (c + 1) % int(lanes)] += 0.10
    p = p - p.mean(dim=-1, keepdim=True)
    return p * float(init_strength)
...
class TapeLaneTransformUnit(nn.Module):
    ...
    def __init__(self, dim: int, lanes: int, cells_per_lane: int, channel_stages: int, dropout: float, context_primitive_scale: float):
        super().__init__()
        self.dim = int(dim)
        self.lanes = int(lanes)
        self.cells_per_lane = int(cells_per_lane)
        self.context_primitive_scale = float(context_primitive_scale)
        ...
        self.context_to_primitive = nn.Linear(dim, len(PRIMITIVES), bias=False)
    ...
    def forward(self, x: torch.Tensor, read_packet: torch.Tensor, lane_embed: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        lane_bias = self.lane_primitive_bias(lane_embed.to(device=x.device, dtype=x.dtype)).float().view(1, lanes, len(PRIMITIVES))
        ctx_summary = read_packet.mean(dim=2)  # [B,L,D]
        ctx_bias = self.context_to_primitive(ctx_summary.to(dtype=x.dtype)).float()
        logits = self.primitive_logits.float().view(1, 1, -1) + lane_bias
        logits = logits + float(self.context_primitive_scale) * ctx_bias
        weights = torch.softmax(logits, dim=-1).to(x.dtype)  # [B,L,P]
        weights_full = weights.view(bsz, lanes, 1, len(PRIMITIVES), 1).expand(-1, -1, cells, -1, -1)
        ...
        return update, {
            "primitive_weights": weights.detach().float().mean(dim=0),  # [L,P]
...
class TapeLaneRouterBackbone(nn.Module):
    ...
    def __init__(
        ...
        channel_stages: int,
        dropout: float,
        structured_init_strength: float,
        read_prior_mode: str,
        read_prior_strength: float,
        route_prior_mode: str,
        route_prior_strength: float,
        boundary_route_strength: float,
        context_primitive_scale: float,
    ):
        super().__init__()
        self.dim = int(dim)
        self.lanes = int(lanes)
        ...
        self.structured_init_strength = float(structured_init_strength)
        self.read_prior_mode = str(read_prior_mode)
        self.route_prior_mode = str(route_prior_mode)
        ...
        route_init = torch.zeros(tape_steps, lanes, lanes)
        if route_prior_mode in {"identity", "weak_flow"}:
            route_init = route_init + torch.eye(lanes).view(1, lanes, lanes) * float(route_prior_strength)
        elif route_prior_mode != "neutral":
            raise ValueError(f"unknown route_prior_mode={route_prior_mode}")
        self.route_logits = nn.Parameter(route_init)

        read_init = torch.zeros(tape_steps, lanes, lanes + 1)
        if read_prior_mode not in {"neutral", "self", "structured"}:
            raise ValueError(f"unknown read_prior_mode={read_prior_mode}")
        if read_prior_mode in {"self", "structured"}:
            for t in range(tape_steps):
                ...
                if read_prior_mode == "structured" and lanes >= 4:
        ...
        read_init = read_init * float(read_prior_strength)
        ...
        if lanes >= 4 and route_prior_mode == "weak_flow":
            bias[1, 2] = 0.30
            bias[1, 3] = 0.18
            bias[0, 1] = 0.12
            bias[3, 1] = 0.10
        self.register_buffer("boundary_route_bias", bias * float(boundary_route_strength))

        self.units = nn.ModuleList([
            TapeLaneTransformUnit(dim, lanes, cells_per_lane, channel_stages, dropout, context_primitive_scale)
    ...
    def _neutral_init(self, evidence: torch.Tensor) -> torch.Tensor:
        lane_states = [self._attention_init_lane(evidence, lane) for lane in range(self.lanes)]
        return torch.stack(lane_states, dim=1)

    def _structured_init(self, evidence: torch.Tensor) -> torch.Tensor:
    ...
    def init_lanes(self, evidence: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        neutral = self._neutral_init(evidence)
        structured = self._structured_init(evidence)
        s = max(0.0, min(1.0, float(self.structured_init_strength)))
        x = (1.0 - s) * neutral + s * structured
...
class ClassMatrixLaneHead(nn.Module):
    """v3 ClassMatrix head adapted from phases to lanes with weak configurable prior."""
    ...
    def __init__(
        ...
        self.class_lane_logits = nn.Parameter(class_lane_prior(classes, lanes, class_lane_prior_mode, class_lane_init_strength))
...
def aux_losses(logits: torch.Tensor, baux: TapeLaneAux, haux: Dict[str, torch.Tensor], args) -> Dict[str, torch.Tensor]:
    ...
    memory_overwrite = F.relu(memory_gate - float(args.memory_write_target)).pow(2)
    ...
    return {
        ...
        "memory_overwrite_cost": memory_overwrite,
        "memory_write_gate": memory_gate.detach(),
...
def write_chatgpt_report(out_dir: Path, analysis: Dict, args) -> None:
    ...
    prior_config = rep.get("prior_config", {})
    ...
    lines = [
        "REPORT_TO_CHATGPT",
        "",
        f"Версия: v4.2 weak-prior/ablation",
...
def run(args) -> None:
    ...
    print(
        f"TapeLaneRouter v4.2 params={params} T={args.tape_steps} lanes={args.lanes} "
    ...
    fields = [
        ...
        "late_input_lambda_effective", "memory_overwrite_cost", "memory_write_gate", "logit_norm", "pair_update_norm",
    ]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {
            ...
            "memory_write_gate": tr.get("memory_write_gate", 0.0),
...
def parser() -> argparse.ArgumentParser:
    ...
    # Weak-prior controls / ablations.
    p.add_argument("--structured-init-strength", type=float, default=0.35)
    p.add_argument("--read-prior-mode", choices=["neutral", "self", "structured"], default="structured")
    p.add_argument("--read-prior-strength", type=float, default=0.35)
    p.add_argument("--route-prior-mode", choices=["neutral", "identity", "weak_flow"], default="weak_flow")
    p.add_argument("--route-prior-strength", type=float, default=0.20)
    p.add_argument("--boundary-route-strength", type=float, default=0.25)
    p.add_argument("--context-primitive-scale", type=float, default=0.15)
    p.add_argument("--class-lane-prior-mode", choices=["neutral", "round_robin", "soft_cover"], default="soft_cover")
    p.add_argument("--class-lane-init-strength", type=float, default=0.10)
    ...
    p.add_argument("--lane-prior-strength", type=float, default=0.20)
    ...
    p.add_argument("--memory-write-target", type=float, default=0.28)
    ...
    p.add_argument("--lambda-memory-overwrite", type=float, default=0.004)
    ...
    p.add_argument("--out-dir", default="./simple_butterfly_matrix_v4_tape_lane/runs/speechcommands_5ep_v4_2")
