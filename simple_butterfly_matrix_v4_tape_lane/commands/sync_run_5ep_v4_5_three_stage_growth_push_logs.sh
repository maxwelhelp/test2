...
# Starts from a small 3-step skeleton:
#   step 0: evidence/local preparation
#   step 1: transform/route/memory work
#   step 2: aggregation/head preparation
# Then reports where the next inserted step should go. This avoids paying for
# 12 exact tape steps before the program has learned where extra depth is useful.
...
REPORT_DIR="${OUT_DIR:-simple_butterfly_matrix_v4_tape_lane/agent_reports/v4_5_three_stage_growth_${TS}}"
mkdir -p "$REPORT_DIR"
...
TAPE_STEPS="${TAPE_STEPS:-3}"
PAIR_SLOTS="${PAIR_SLOTS:-6}"
CELLS_PER_LANE="${CELLS_PER_LANE:-10}"
EVIDENCE_CELLS="${EVIDENCE_CELLS:-40}"

# Scout stays weak/paid. For this first growth run, offline feedback is optional.
SCOUT_FEEDBACK_PATH="${SCOUT_FEEDBACK_PATH:-}"
SCOUT_FEEDBACK_SCALE="${SCOUT_FEEDBACK_SCALE:-0.008}"
ONLINE_SCOUT_SCALE="${ONLINE_SCOUT_SCALE:-0.012}"
ONLINE_SCOUT_GATE_INIT="${ONLINE_SCOUT_GATE_INIT:--3.2}"
ONLINE_SCOUT_USE_COST="${ONLINE_SCOUT_USE_COST:-0.0008}"
...
python "$MAIN" \
  ...
  --compare-to "v4.5_three_stage_growth skeleton T=$TAPE_STEPS" \
  --profile-speed \
  --pin-memory \
  --log-every "$LOG_EVERY" \
  --no-save-checkpoints \
...
# Optional planner: if the model ran, propose where to insert the next step.
...
if ! git diff --cached --quiet; then
  git commit -m "Add v4.5 three-stage growth run logs $TS"
  git push
fi
exit "$RUN_STATUS"
